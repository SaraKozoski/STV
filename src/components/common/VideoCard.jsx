...
const VideoCard = ({ video, featured = false }) => {
  ...
  const getThumbnail = () => `https://img.youtube.com/vi/${video.youtube_id}/maxresdefault.jpg`;
  ...
  return (
    <>
      ...
      {!featured && (
        <div
          onClick={openModal}
          className="flex sm:hidden items-center gap-3 p-3 bg-white rounded-xl shadow-sm hover:shadow-md transition-all cursor-pointer group"
        >
          <div className="relative w-20 h-20 flex-shrink-0 overflow-hidden rounded-lg bg-gray-200">
            <ThumbnailImg className="group-hover:scale-105 transition-transform duration-300" />
          ...
          <div className="flex-1 min-w-0">
            {video.categories && (
              <span className="text-primary-500 font-medium text-xs">{getCategoryName()}</span>
            )}
            <h3 className="font-bold text-gray-900 text-sm line-clamp-3 group-hover:text-primary-500 transition-colors mt-0.5">
              {getTitle()}
            </h3>
          </div>
        </div>
          ...
          >
            ...
            <div className="p-6 md:p-8">
              ...
              <div className="flex flex-wrap items-center gap-3 mb-6">
                ...
                {video.created_at && (
                  <span className="text-gray-500 text-sm">📅 {formatDate(video.created_at)}</span>
                )}
